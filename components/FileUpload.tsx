...
const STEPS = [
  { icon: fileuploadicon, label: 'Upload STL File', description: 'Upload your 3D model file' },
  { icon: processing, label: 'Processing', description: 'Analyzing model and preparing for print' },
  { icon: quote, label: 'Quote Ready', description: 'Review your custom quote' },
  { icon: cart, label: 'Add to cart', description: 'Confirm details and add to cart' },
  { icon: orderplaced, label: 'Order Placed', description: 'Your model is being printed' },
  { icon: delivered, label: 'Delivered', description: 'Arrives in about 5 business days' }
]
...
export function FileUpload({ 
  ...
  const [fileInputError, setFileInputError] = useState<"multiple_files" | "file_size" | "wrong_type" | "upload_error" | null>(null)
  ...
  const handleDragOver = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (e.dataTransfer.items && e.dataTransfer.items.length > 0) {
      ...
      const areFilesAccepted = draggedFiles.every(item => 
        item.kind === 'file' && acceptedMimeTypes.includes(item.type)
      );
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length === 1) {
      ...
      if(droppedFile.size > 50 * 1024 * 1024){ // 50MB size
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length === 1) {
      ...
      if(selectedFile.size > FILE_SIZE_CAP * 1024 * 1024){ // 50MB size
        setFileInputError("file_size");
        return
      }
                  ...
                  {isDragging ? (
                    ...
                    <div>
                      <p className="text-base flex gap-1 items-center text-red-500/90">
                        <AiOutlineExclamationCircle /> You can only upload one file
                      </p>
                      <p className="text-sm text-gray-500 mt-2">
                        Maximum file size: 50MB
                      </p>
                    ...
                    <div>
                      <p className="text-base flex gap-1 items-center text-red-500/90">
                        <AiOutlineExclamationCircle /> The file size must be less than {FILE_SIZE_CAP} MB
                      </p>
                      <p className="text-sm text-gray-500 mt-2">
                        Maximum file size: 50MB
                      </p>
                    ...
                    <div>
                      <p className="text-base flex gap-1 items-center text-red-500/90">
                        <AiOutlineExclamationCircle /> Upload failed, please try again
                      </p>
                      <p className="text-sm text-gray-500 mt-2">
                        Maximum file size: 50MB
                      </p>
                    ...
                    <div>
                      <p className="text-base flex gap-1 items-center text-red-500/90">
                        <AiOutlineExclamationCircle /> The file must have .stl extension
                      </p>
                      <p className="text-sm text-gray-500 mt-2">
                        Maximum file size: 50MB
                      </p>
                    ...
                    <div>
                      <p className="text-base text-gray-500">
                        Click to upload <span className='text-gray-500/80'>or drag and drop</span>
                      </p>
                      <p className="text-sm text-gray-500 mt-2">
                        Maximum file size: 50MB
                      </p>
                ...
                <div className="mt-4 p-4 bg-gray-50 rounded-sm items-center justify-between border border-neutral-300">
                  <div className='flex justify-between'>
                    <div className="flex items-center gap-3">
                      <Image src={stl_file_icon} alt="File Icon" className='w-[25px]'/>
                      <div>
                    ...
                    <div className='flex justify-between items-center pb-6'>
                      <div
                        className={`flex flex-1 items-center gap-4 px-4 py-2 pr-5 max-w-7/9 rounded-lg transition-all duration-300 ease-out transform
                          ${isActive ? 'bg-neutral-300/10 border border-neutral-300 scale-103 translate-x-1' :
                          isCompleted ? 'bg-neutral-300/10 border border-neutral-200' :
                          'bg-gray-50'}`}
                        >
                        <div className={`rounded-full p-2 transition-all duration-300 ease-out transform
                          ${isActive ? 'bg-neutral-300/40 text-neutral-300 scale-110' :
                            isCompleted ? 'bg-neutral-400/20 text-neutral-500/90 scale-90' :
                            'bg-gray-100 text-gray-400'}`}
                        >
                          {isCompleted ? <IoMdCheckmark className="w-[16px]"/> : <Image src={Icon} alt='' className='w-[16px]'/>}
                        </div>
                        <div>
                      ...
                      <div className="relative flex flex-col items-center">
                        <div className={`rounded-full border border-neutral-200/70 w-8 h-8 flex items-center justify-center font-medium text-xs z-10 transition-all duration-300 ease-out transform
                          ${isActive ? 'bg-neutral-100 border border-neutral-300 scale-105' :
                            isCompleted ? 'bg-neutral-50 border border-neutral-200' :
                            'bg-white'} cursor-default`}>
                          {index + 1}
                        </div>
                        {!isLastStep && (
                          <div className={`absolute top-8 w-[1px] h-16 transition-all duration-700 ease-in-out ${
                            isCompleted ? 'bg-neutral-300' : 'bg-neutral-200/60'
                          }`} />
                        )}
                      </div>
                    </div>
